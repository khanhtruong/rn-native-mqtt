...
class Client {
    ...
    willmessage(topic, message, qos = 0, retained = false) {
      ...
      NativeMqtt.willmessage(this.id, topic, hex ? result : typeof message !== String ? message: stringToBytes(message) , qos, retained);  
    }
    ...
    publish(topic, message, qos = 0, retained = false) {
      ...
      NativeMqtt.publish(this.id, topic, hex ? result : typeof message !== String ? message: stringToBytes(message) , qos, retained);  
    }
    disconnect() {
        if (this.closed) {
            throw new Error('client already closed');
